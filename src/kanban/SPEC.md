...
|---|---|---|
| Single `.jsx` file under `src/<name>/src/<Component>.jsx` | Both | `src/kanban/src/Kanban.jsx` |
| Hand-written `index.d.ts` at package root | Both | `src/kanban/index.d.ts` |
...
interface KanbanSortOption<Row> {
  value: string;
  label: string;
  field?: string;         // optional metadata for shared table/board sort configs
  direction?: "asc" | "desc";
...
Sort is a single-select dropdown (one sort applies to every column) because that matches native Deals behavior and avoids the combinatoric explosion of per-column sort. The toolbar displays the option `label` values in a compact transparent `Select`, matching the Feed toolbar style.
...
interface KanbanLabels {
  ...
  sortButton?: string;                     // "Sort" — Select placeholder on toolbar
  sortAscending?: string;                  // deprecated; retained for compatibility
  sortDescending?: string;                 // deprecated; retained for compatibility
