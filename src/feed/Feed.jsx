...
import { CollectionCount } from "../common-components/CollectionCount.js";
...
const getDefaultFilterValue = (filter) => {
  ...
  if (filter.type === "dateRange") return { from: null, to: null };
  return filter.includeAll === false ? "" : "all";
};
...
const DefaultFeedItem = ({
  ...
  const titleText = hasValue(title) ? (
...
const FeedToolbar = ({
  ...
  const resolvedFilterValues = Object.fromEntries(
    (filters || []).map((filter) => [filter.name, getFilterValue(filterValues, filter)])
  );

  const rightControls = !hideRightControls && ((Array.isArray(sortOptions) && sortOptions.length > 0) || showItemCount) ? (
    <>
      {Array.isArray(sortOptions) && sortOptions.length > 0 ? (
        <CollectionSortSelect
          name="feed-sort"
          value={sort}
          placeholder={labels.sortButton || labels.sort}
          options={sortOptions}
          onChange={onSort}
        />
      ) : null}
      {showItemCount ? <CollectionCount text={itemCountLabel} /> : null}
    </>
  ) : null;
  ...
  return (
    <CollectionToolbar
      ...
      filters={{
        ...
        namePrefix: "feed-filter",
        onChange: onFilter,
        allValue: "all",
        overflowButtonSize: "sm",
        filtersButtonLabel: labels.filtersButton || "Filters",
        labels,
      }}
      chips={{
        items: activeChips,
...
export const Feed = ({
  ...
  const handleFilterRemove = (name) => {
    const nextFilters = resetFilterValues(filters, activeFilters, name, {
      getEmptyValue: getDefaultFilterValue,
    });
    if (filterValues === undefined) setInternalFilters(nextFilters);
    setVisibleCount(initialItemCount ?? pageSize);
    onFilterChange?.(nextFilters);
  ...
  const activeChips = useMemo(() => {
    const chipValues = Object.fromEntries(
      (filters || []).map((filter) => [filter.name, getFilterValue(activeFilters, filter)])
    );
    return buildActiveFilterChips(filters, chipValues, {
      isFilterActive: (_filter, value) => !isEmptyFilterValue(value),
  ...
  // The toolbar's right-side controls (sort + item count) ride on the first
  // date-group header row instead of floating in a standalone row above it —
  // mirroring HubSpot's activity timeline, where "Newest first / N events"
  // sits on the same line as the first "Today" group. Search / filters (if
  // any) stay on the toolbar's left; only the right side moves down.
  const sortControl = (Array.isArray(sortOptions) && sortOptions.length > 0) ? (
    <CollectionSortSelect
      name="feed-sort"
      value={activeSort}
      placeholder={labels.sortButton || labels.sort}
  ...
  const countControl = showItemCount ? <CollectionCount text={itemCountLabel} /> : null;
  const toolbarRight = (sortControl || countControl) ? (
    <Inline gap="sm" align="center">{sortControl}{countControl}</Inline>
  ) : null;
  ...
  const alignControlsWithFirstGroup =
    !renderToolbar &&
    showToolbar &&
    !loading &&
    !error &&
    processedItems.length > 0 &&
    !hasLeftToolbarControls &&
    !!toolbarRight &&
    firstGroupHasLabel &&
    (alignToolbarWithGroups === true || (alignToolbarWithGroups === "auto" && (groupByDate || !!groupBy)));
  ...
  const toolbarNode = renderToolbar ? renderToolbar({
    ...
    showToolbar && !loading && !error ? (
      <FeedToolbar
        hideRightControls={alignControlsWithFirstGroup}
        labels={labels}
        search={activeSearch}
  ...
  const collapseToggle = (showCollapseToggle && collapsibleVisibleItems.length > 1 && !loading && !error) ? (
    <Link onClick={allCollapsed ? handleExpandAll : handleCollapseAll}>
      {keepWordsTogether(allCollapsed ? labels.expandAll : labels.collapseAll)}
    </Link>
  ) : null;
      ...
      <Flex key="items" direction="column" gap={compact ? "xs" : gap}>
        {groups.map((group, groupIndex) => (
          <Flex key={group.key} direction="column" gap={compact ? "xs" : gap}>
            {hasValue(group.label) && (
              alignControlsWithFirstGroup && groupIndex === 0 ? (
                <Flex direction="row" justify="between" align="center" gap="sm">
                  <Text format={{ fontWeight: "demibold" }}>{group.label}</Text>
                  {toolbarRight}
                </Flex>
              ) : (
                <Text format={{ fontWeight: "demibold" }}>{group.label}</Text>
